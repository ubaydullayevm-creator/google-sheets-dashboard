...
function parseSalesCSV(csvText) {
    ...
    for (let i = 1; i < lines.length; i++) {
        const line = lines[i];
        // Регулярное выражение для более надежного парсинга CSV с кавычками
...
// ======================================================================================
// === ОБЪЕДИНЕНИЕ ДАННЫХ И ЛОГИКА ТЕРРИТОРИЙ (ОБНОВЛЕНО) ===
// ======================================================================================

/** ОБНОВЛЕНО: Добавлено хранение Target по Парентам */
function combineData(targetsByGroup, targetsByParent, salesAggregated, salesDetailed) {
    ...
    combined.allParentTargets = targetsByParent; // Храним Targets по Парентам
    return combined;
}

/** ОБНОВЛЕНО: Агрегация данных по Территориям теперь использует детализированный Target */
...
// ======================================================================================
// === ОТОБРАЖЕНИЕ ДАННЫХ (ГРУППЫ И ТЕРРИТОРИИ - ОБНОВЛЕНО) ===
// ======================================================================================

/** ОБНОВЛЕНО: Возвращает итоговые значения */
...
/** ОБНОВЛЕНО: Отображение данных по Территориям с расчетом Target/Execution */
function displayTerritoryData(aggregatedData, totalTargetKPI, totalSalesKPI) {
    ...
    const sortedTerritories = Object.keys(aggregatedData).sort((a, b) => {
        // Сортируем по Target, если Sales равны
    ...
    // Для KPI используем Total Target и Total Sales, переданные из расчетов Групп
    ...
    // Обновление KPI (используем итоги по Группам для согласованности с верхним уровнем)
...
function renderTerritoryChart(aggregatedData) {
    ...
    // Сортируем по убыванию Sales для диаграммы
    const sortedTerritories = Object.keys(aggregatedData).sort((a, b) => {
        return aggregatedData[b].sales - aggregatedData[a].sales;
    });
...
// ======================================================================================
// === ОСНОВНОЙ КОНТРОЛЛЕР И ИНИЦИАЛИЗАЦИЯ (ОБНОВЛЕНО) ===
// ======================================================================================

/** ОБНОВЛЕНО: Обновление дашборда после загрузки или фильтрации */
function updateDashboard(combinedData) {
    ...
    if (selectedFilterGroup !== 'All') {
        // При фильтре по группе показываем только одну группу
        const temp = {};
...
/** ОБНОВЛЕНО: Функция загрузки данных по двум URL */
async function fetchData() {
    try {
        ...
        const targetsResult = parseTargetCSV(targetCSV); // Возвращает Targets по Группам И Парентам
